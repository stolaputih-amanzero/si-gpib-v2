...
export function AssetIntelligenceClient({ assetData }: AssetIntelligenceClientProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-surface-base pb-24">
      {/* Header */}
      <header className="bg-surface-elevated border-b border-border-subtle pt-12 pb-4 px-4 sticky top-0 z-20">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-text-muted capitalize">
              Proyeksi Aset Lintas Konteks • {assetData.orgName} ({assetData.orgLevel})
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-4">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <Card className="bg-surface-elevated border-border-subtle">
              <CardContent className="p-4 flex flex-col items-center text-center">
                ...
                <span className="text-2xl font-bold text-text-high">{assetData.summary.totalTanah}</span>
                <span className="text-xs text-text-muted uppercase tracking-wider">Bidang Tanah</span>
              </CardContent>
            </Card>
            <Card className="bg-surface-elevated border-border-subtle">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <Building className="text-brand-secondary mb-2" size={24} />
                <span className="text-2xl font-bold text-text-high">{assetData.summary.totalBangunan}</span>
                <span className="text-xs text-text-muted uppercase tracking-wider">Bangunan</span>
              </CardContent>
            </Card>
            <Card className="bg-surface-elevated border-border-subtle">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <Car className="text-brand-tertiary mb-2" size={24} />
                <span className="text-2xl font-bold text-text-high">{assetData.summary.totalBergerak}</span>
                <span className="text-xs text-text-muted uppercase tracking-wider">Aset Bergerak</span>
              </CardContent>
            </Card>
            <Card className="bg-surface-elevated border-border-subtle">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <FileText className="text-state-info mb-2" size={24} />
                <span className="text-2xl font-bold text-text-high">{assetData.summary.totalLampiran}</span>
                <span className="text-xs text-text-muted uppercase tracking-wider">Legalitas</span>
              </CardContent>
            </Card>
          </div>
        )}
        ...
        {assetData.orgLevel !== 'POS' && assetData.children && assetData.children.length > 0 && (
          <div className="mb-8">
            <h2 className="text-sm font-bold text-text-high uppercase tracking-wider mb-3">Distribusi per Pos Pelkes</h2>
            <div className="space-y-2">
              {assetData.children.map((child: any) => (
                <Link href={`/assets/${child.id}`} key={child.id}>
                  <Card className="bg-surface-elevated border-border-subtle active:scale-[0.98] transition-transform">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold text-text-high">{child.name}</h3>
                        <p className="text-xs text-text-muted mt-1 flex gap-3">
            ...
            {[
              ...
              <button
                ...
                className={`pb-3 flex items-center gap-2 text-sm font-medium transition-colors relative whitespace-nowrap
                  ${activeTab === tab.id ? 'text-brand-primary' : 'text-text-muted hover:text-text-high'}`}
              >
                <tab.icon size={16} />
                {tab.label}
                {activeTab === tab.id && (
              ...
              {assetData.tanah.length === 0 ? (
                ...
                assetData.tanah.map((item: any) => (
                  <Link href={`/assets/${item.id_tanah}`} key={item.id_tanah} className="block">
                    <Card className="mb-3 bg-surface-elevated border-border-subtle hover:border-brand-primary/50 transition-colors">
                      <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-semibold text-text-high">{item.m_pos_pelkes?.nama_pos || assetData.orgName}</h3>
              ...
              {assetData.bergerak.length === 0 ? (
                ...
                assetData.bergerak.map((item: any) => (
                  <Link href={`/assets/${item.id_aset_b}`} key={item.id_aset_b} className="block">
                    <Card className="mb-3 bg-surface-elevated border-border-subtle hover:border-brand-tertiary/50 transition-colors">
                      <CardContent className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-semibold text-text-high">{item.m_pos_pelkes?.nama_pos || assetData.orgName}</h3>
...
function EmptyState({ icon: Icon, title, subtitle }: { icon: any, title: string, subtitle?: string }) {
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-surface-elevated border border-dashed border-border-subtle rounded-xl">
      <div className="w-12 h-12 bg-surface-sunken rounded-full flex items-center justify-center text-text-muted mb-3">
        <Icon size={24} />
      </div>
      <h3 className="font-medium text-text-high mb-1">{title}</h3>
      <p className="text-sm text-text-muted">{subtitle || 'Tidak ada data untuk kategori ini'}</p>
    </div>
  );
}

...
import { ChevronRight, ArrowLeft, Home, Layers } from 'lucide-react';
...
export function MobileHeaderBreadcrumb() {
  ...
  return (
    <header className="md:hidden sticky top-0 z-40 w-full backdrop-blur-md bg-surface-elevated/95 border-b border-border-subtle shadow-soft select-none shrink-0">
      <div className="flex items-center h-14 px-3.5 gap-2 overflow-hidden">
          ...
          {crumbs.map((crumb, index) => {
            ...
            return (
              <div key={`${crumb.href}-${index}`} className="flex items-center gap-1 shrink-0">
                ...
                {isLast ? (
                  <span className="font-extrabold text-brand-primary bg-brand-primary/10 px-2 py-1 rounded-lg text-xs tracking-tight truncate max-w-[150px]">
                    {crumb.label}
                  </span>
                ) : (
                  <Link

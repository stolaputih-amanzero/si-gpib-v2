      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-surface-elevated border border-border-subtle shadow-xs space-y-3">
          ...
          {competencies.skills.type === 'DATA' ? (
            ...
            <p className="text-xs italic text-text-disabled p-3 text-center bg-surface-sunken rounded-xl border border-border-subtle">
          ...
          {competencies.education.type === 'DATA' ? (
            <div className="space-y-2">
              {competencies.education.value.map((edu, i) => (
                <div key={i} className="p-3 rounded-xl bg-surface-sunken text-xs space-y-0.5 border border-border-subtle">
                  <p className="font-bold text-text-high">{edu.jenjang} {edu.jurusan ? `- ${edu.jurusan}` : ''}</p>
          ...
          {competencies.certifications.type === 'DATA' ? (
            <div className="space-y-2">
              {competencies.certifications.value.map((cert, i) => (
                <div key={i} className="p-3 rounded-xl bg-surface-sunken text-xs space-y-0.5 border border-border-subtle">
                  <p className="font-bold text-text-high">{cert.nama_sertifikasi}</p>

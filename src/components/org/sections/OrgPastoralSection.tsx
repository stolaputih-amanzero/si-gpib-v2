      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 border border-emerald-500/30 transition-colors min-h-[44px]"
          ...
          {pastoral.canCreate && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-brand-primary hover:bg-brand-primary/90 text-white shadow-xs transition-colors min-h-[44px]"
      ...
      <div className="bg-surface-elevated border border-border-subtle rounded-2xl p-5 shadow-xs space-y-5">
        ...
        <div className="space-y-2">
          ...
          {pastoral.jadwalIbadah.type === 'PRIVACY_MASKED' ? (
            ...
            <p className="text-xs text-text-disabled italic py-1">{pastoral.jadwalIbadah.label}</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {pastoral.jadwalIbadah.value.map((item) => (
                <div key={item.id_jadwal} className="p-3.5 rounded-xl bg-surface-sunken border border-border-subtle text-xs flex justify-between items-center">
                  <div>
                    <div className="font-bold text-text-high">{item.nama_ibadah}</div>
                    <div className="text-text-muted mt-0.5">{item.hari}</div>
                  </div>
                  <span className="px-2.5 py-1 rounded-lg bg-brand-primary/10 text-brand-primary font-bold border border-brand-primary/20 font-sans tabular-nums flex items-center gap-1">
        ...
        <div className="space-y-2 pt-3 border-t border-border-subtle">
          ...
          {pastoral.logs.type === 'PRIVACY_MASKED' ? (
            ...
            <p className="text-xs text-text-disabled italic py-1">{pastoral.logs.label}</p>
          ) : (
            <div className="space-y-2">
                ...
                >
                  <div className="min-w-0 pr-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-sm font-bold text-text-high group-hover:text-brand-primary transition-colors">
                        {log.jenis_kegiatan}
                      </span>
                      <span className="text-xs font-bold text-text-muted font-sans tabular-nums bg-surface-sunken px-2 py-0.5 rounded border border-border-subtle">
                        {log.tgl}
                      </span>
                    </div>

  ...
  return (
    <div className="min-h-screen bg-[#0B1220] pb-24">
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6 space-y-6">
        ...
        <div className="p-6 rounded-2xl bg-slate-900/90 border border-border-subtle shadow-xs space-y-3 relative overflow-hidden">
          ...
          <div className="w-full h-48 rounded-xl bg-slate-950 border border-border-subtle flex flex-col items-center justify-center p-4 text-center space-y-2">
        ...
        <div className="space-y-3">
          ...
          {MOCK_TERRITORY_RISKS.map((item) => (
            <div
              key={item.id}
              className="p-5 rounded-2xl bg-slate-900/90 border border-border-subtle shadow-xs space-y-3 hover:border-blue-500/40 transition-all group"
            >
              <div className="flex items-center justify-between border-b border-border-subtle pb-3">
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-blue-400 shrink-0" />
                  <span className="font-bold text-slate-100 text-sm group-hover:text-blue-400 transition-colors">{item.wilayah}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${
                    item.tingkat === 'Tinggi' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                  }`}>
                    Resiko {item.tingkat}
                  </span>
                  ...
                  <Link
                    ...
                    className="p-2 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 text-xs font-bold flex items-center gap-1 min-h-[44px]"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-xl bg-slate-950 border border-border-subtle space-y-1">
                  <span className="font-bold text-amber-400 flex items-center gap-1">
                    <AlertTriangle className="w-3.5 h-3.5" /> Kerawanan Dominan:
                  </span>
                  <p className="text-slate-300 font-medium">{item.jenis}</p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-slate-950 border border-border-subtle space-y-1">
                  ...
                  <p className="text-slate-300 font-medium">{item.sda} • {item.sdm}</p>
                </div>
              </div>
            </div>
          ))}
